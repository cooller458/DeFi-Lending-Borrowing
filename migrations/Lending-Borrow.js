const BLNToken = artifacts.require('BLNToken');
const BLN2Token = artifacts.require("BLN2");

const LendingAndBorrowing = artifacts.require("LendingAndBorrowing");
const tools = require("../helpcenter/helpful_scripts");
...
module.exports = async function(deployer,network,accounts) {
    await deployer.deploy(BLNToken)
    await deployer.deploy(BLN2)
    const bln_token = await BLNToken.deployed()
    await deployer.deploy(LendingAndBorrowing,bln_token.address)
    const lending_and_borrowing = await LendingAndBorrowing.deployed()

    dai_token_address = await tools.get_contract("dai_token_address", newtork,deployer)
    weth_token_address = await tools.get_contract("weth_token_address", newtork,deployer)
    link_token_address = await tools.get_contract("link_token_address",network,deployer)
    fau_token_address = await tools.get_contract("face_token_address",network,deployer)

    const token_to_price_feed  = await addTokenToPriceFeed(network, deployer)

    var total = await bln_token.totalSupply()
    tx = await bln_token.transfer(lending_and_borrowing.address,BigInt(total)-BigInt(KEPT_BALANCE))
...
const addTokenToPriceFeed = async (network, deployer) => {
    dai_usd_price_feed_address = await tools.get_contract("dai_usd_price_feed_address", network,deployer)
        weth_usd_price_feed_address = await tools.get_contract("dai_usd_price_feed_address", network,deployer)
        link_usd_price_feed_address = await tools.get_contract("link_usd_price_feed_address", network,deployer)
        fau_usd_price_feed_address = await tool.get_contract("fau_usd_price_feed_address", network,deployer)
